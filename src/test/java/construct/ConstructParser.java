...
public class ConstructParser {
	...
	public FreeProtocolHistory makeFreeProtocol(OpReturn opre, Protocol prot) {
		// TODO Auto-generated method stub
		Gson gson = new Gson();
		FreeProtocolRaw freeProtocolRaw = new FreeProtocolRaw ();
		try {
		freeProtocolRaw = gson.fromJson(gson.toJson(prot.getData()),FreeProtocolRaw.class);
		}catch(com.google.gson.JsonSyntaxException e) {
			return null;
		}
		if(freeProtocolRaw==null)return null;
	...
	public ServiceHistory makeService(OpReturn opre, Protocol prot) {
		// TODO Auto-generated method stub
		Gson gson = new Gson();
		ServiceRaw serviceRaw = new ServiceRaw();
		
		try {
			serviceRaw = gson.fromJson(gson.toJson(prot.getData()),ServiceRaw.class);
		}catch(com.google.gson.JsonSyntaxException e) {
			return null;
		}
	...
	public AppHistory makeApp(OpReturn opre, Protocol prot) {
		// TODO Auto-generated method stub
		Gson gson = new Gson();
		...
		try {
			appRaw = gson.fromJson(gson.toJson(prot.getData()),AppRaw.class);
		}catch(com.google.gson.JsonSyntaxException e) {
			return null;
		}
	...
	public CodeHistory makeCode(OpReturn opre, Protocol prot) {
		// TODO Auto-generated method stub
		Gson gson = new Gson();
		CodeRaw codeRaw = new CodeRaw();
		
		try {
			codeRaw = gson.fromJson(gson.toJson(prot.getData()),CodeRaw.class);
		}catch(com.google.gson.JsonSyntaxException e) {
			return null;
		}
	...
	public boolean parseFreeProtocol(ElasticsearchClient esClient, FreeProtocolHistory freeProtocolHist) throws ElasticsearchException, IOException {
		...
		case "publish":			
			...
			if(freeProtocol==null) {
				...
				freeProtocol.setFirstTxid(freeProtocolHist.getId());
				freeProtocol.setFirstTime(freeProtocolHist.getTime());
				freeProtocol.setFirstHeight(freeProtocolHist.getHeight());
	...
	public boolean parseService(ElasticsearchClient esClient, ServiceHistory serviceHist) throws ElasticsearchException, IOException, InterruptedException {
		...
		case "publish":
			...
			if(service==null) {
				...
				service.setFirstTime(serviceHist.getTime());
				service.setFirstHeight(serviceHist.getHeight());
	...
	public boolean parseApp(ElasticsearchClient esClient, construct.AppHistory appHist) throws ElasticsearchException, IOException {
		...
		case "publish":
			...
			if(app==null) {
				...
				app.setFirstTime(appHist.getTime());
				app.setFirstHeight(appHist.getHeight());
	...
	public boolean parseCode(ElasticsearchClient esClient, CodeHistory codeHist) throws ElasticsearchException, IOException {
		...
		case "publish":
			code = EsTools.getById(esClient, Indices.CodeIndex, codeHist.getCodeId(), Code.class);		
			if(code==null) {
				code = new Code();
				code.setCodeId(codeHist.getId());
				...
				code.setFirstTime(codeHist.getTime());
				code.setFirstHeight(codeHist.getHeight());
				...
				Code code1=code;
				esClient.index(i->i.index(Indices.CodeIndex).id(codeHist.getCodeId()).document(code1));
				isValid = true;
			}else {
				isValid = false;
			}
		...
		case "stop":
			...
			if(code.isActive()) {
				...
				esClient.index(i->i.index(Indices.CodeIndex).id(codeHist.getCodeId()).document(app2));
				isValid = true;
			}else isValid = false;
		...
		case "recover":
			...
			if(!code.isActive()) {
				...
				esClient.index(i->i.index(Indices.CodeIndex).id(codeHist.getCodeId()).document(code2));
				isValid = true;
			}else isValid = false;
		...
		case "rate":
			...
			esClient.index(i->i.index(Indices.CodeIndex).id(codeHist.getCodeId()).document(code3));
			isValid = true;
			break;
		}
		return isValid;
